...
describe('UnifarmNFTManagerUpgradeableUnlock - Updated timeline', () => {
  var mocks: Mock;
  ...
  describe('NFT mints and burns', () => {
    ...
    it('mint without booster', async () => {
      const amount = unitParser(40000);
      await nftManager.connect(users[2]).stakeOnUnifarm(cohort.address, ZERO_ADDRESS, mocks.UFARM.address, amount, 5);
    });

    it('mint with booster', async () => {
      const amount = unitParser(40000);
      await nftManager.connect(users[3]).stakeAndBuyBoosterPackOnUnifarm(cohort.address, ZERO_ADDRESS, mocks.UFARM.address, 1, amount, 5);
    });

    it('mint with booster', async () => {
      const amount = unitParser(40000);
      await nftManager.connect(users[4]).stakeAndBuyBoosterPackOnUnifarm(cohort.address, ZERO_ADDRESS, mocks.UFARM.address, 1, amount, 5);
    });

    it('burn', async () => {
      // const user = nfts.filter((e) => String(e?.user).toLowerCase() === users[t].address.toLowerCase())[0];
      await nftManager.connect(users[3]).unstakeOnUnifarm(2, { value: feeAmount });
    });
    ...
    // it('mint without booster', async () => {
    //   const amount = unitParser(40000);
    //   await nftManager.connect(users[5]).stakeAndBuyBoosterPackOnUnifarm(cohort.address, ZERO_ADDRESS, mocks.UFARM.address, 1, amount, 5);
    // });
    ...
    it('burn', async () => {
      // const user = nfts.filter((e) => String(e?.user).toLowerCase() === users[t].address.toLowerCase())[0];
      await nftManager.connect(users[2]).unstakeOnUnifarm(1, { value: feeAmount });
    });
    ...
    it('burn', async () => {
      // const user = nfts.filter((e) => String(e?.user).toLowerCase() === users[t].address.toLowerCase())[0];
      await nftManager.connect(users[4]).unstakeOnUnifarm(3, { value: feeAmount });
    });
  });
});
